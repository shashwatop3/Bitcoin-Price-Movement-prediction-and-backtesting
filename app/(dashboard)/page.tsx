...
import { lastNMonthsProfit } from "@/utils/lastNDaysProfit";
...
import { openPositionColumns } from "@/app/(dashboard)/openPositioncolumns";
...
export default function Home() {
  ...
  const barChartData = useMemo(
    () => lastNMonthsProfit(portfolio_data, 7),
    [portfolio_data]
  );
  ...
  const openPosition_data = useMemo(() => {
    if (!backtestData?.Open_Positions) return [];
    return backtestData.Open_Positions
      .slice()
      .reverse()
      .map((position: any) => {
        return {
  ...
  console.log("openPosition_data", openPosition_data);
  ...
  const sectCardData = useMemo(
    () => ({
      ...
      days: state.days,
      transaction_fee_rate: state.transaction_fee_rate,
    }),
    [
      profit,
      winRate,
    ...
    <>
      ...
      <div className="h-full">
        ...
        <div className="md:col-span-12 col-span-12 ">
          ...
          <div className="md:col-span-4 col-span-12">
            <div className="shadow-lg rounded-lg">
              <BarChartComponent title="Monthly Profits" data={barChartData} />
            </div>
          </div>
        </div>
        <div className="bg-white container mx-auto py-2 shadow-lg rounded-lg my-4">
          <DataTable columns={openPositionColumns} data={openPosition_data} title="Open Positions"/>
        </div>
        <div className="bg-white container mx-auto py-2 shadow-lg rounded-lg mb-4">
          <DataTable columns={columns} data={transaction_data} title="Closed Positions"/>
        </div>
