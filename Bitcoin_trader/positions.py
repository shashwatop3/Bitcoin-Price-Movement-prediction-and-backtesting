...
class Positions:
    def __init__(self, initial_capital: float, transaction_fee_rate: float = 0.005, traling_stop_loss: bool = False, trailing_take_profit: bool = False) -> None:
    ...
    def add_position(self, position: Union[BuyOrder, SellOrder]) -> None:
        ...
        elif isinstance(position, SellOrder) and self.available_capital >= transaction_fee:
            self.available_capital -= position_cost
            self.available_capital -= transaction_fee 
            self.current_capital -= transaction_fee
            self.profit_loss -= transaction_fee
            self.positions.append(position)
        else:
