...
class BacktestPayload(BaseModel):
    ...
    Quantity: float
    last_n: int
    transaction_fee_rate: float = 0.005  # Default transaction fee rate of 0.5%
...
model = joblib.load('/app/models/xgboost_model7.joblib')
...
def backtest(payload: BacktestPayload):
    ...
    transaction_fee_rate = payload.transaction_fee_rate
    ...
    positions = Positions(initial_capital, transaction_fee_rate)
    
    for i, (price, signal, date) in enumerate(zip(prices, signals, dates)):

        positions.check_positions(price, date=date)
        
        try:
            if signal == 0:

                buy_order = BuyOrder(entry_price=price, quantity=quantity, stoploss=stop_loss, takeprofit=take_profit, entry_date=date)
                positions.add_position(buy_order)
            elif signal == 2:

                sell_order = SellOrder(entry_price=price, quantity=quantity, stoploss=stop_loss, takeprofit=take_profit, entry_date=date)
...
def get_recent_backtest(days: int = Query(180, description="Number of days to include in the backtest"),
                        stop_loss: float = Query(0.005, description="Stop loss percentage"),
                        take_profit: float = Query(0.018, description="Take profit percentage"),
                        ...
                        transaction_fee_rate: float = Query(0.005, description="Transaction fee rate")):
    
    df = pd.read_csv('/app/data/btc_15m_data_2018_to_2025.csv')
    data = df.tail(96 * (days + 10))
